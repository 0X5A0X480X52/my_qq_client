package cn.amatrix.controller.chatDemo.commponent;

import javax.swing.*;

import com.formdev.flatlaf.ui.FlatRoundBorder;

import java.awt.*;
import java.io.IOException;
import java.awt.image.BufferedImage;
import cn.amatrix.utils.base64.ImageManager;
import java.awt.event.ComponentAdapter;
import java.awt.event.ComponentEvent;

public class MessagePanel extends JPanel {

    private JPanel parentPanel;
    private String avatar;
    private String message;
    private String userName;

    private JLabel avatarIcon;
    private JLabel nameLabel;
    private JTextArea textArea;
    private JPanel avatarPanel;
    private JPanel textPanel;

    public enum Type { OWN, OTHER }
    private Type type = Type.OWN;

    public MessagePanel( String avatar, String message, String userName, JPanel parentPanel) throws IOException {

        this.avatar = avatar;
        this.message = message;
        this.userName = userName;
        this.parentPanel = parentPanel;

        this.setLayout(new BorderLayout());

        this.initAvotorIcon(avatar);
        this.initNameLabel(userName);
        this.initTextArea(message);

        // 创建子Panel用于放置circleLabel
        avatarPanel = new JPanel();
        avatarPanel.setLayout(new BorderLayout());
        avatarPanel.add(avatarIcon, BorderLayout.NORTH);

        // 创建子Panel用于放置textArea
        textPanel = new JPanel();
        textPanel.setLayout(new BoxLayout(textPanel, BoxLayout.Y_AXIS));
        textPanel.setOpaque(true); // 确保 textPanel 是可见的
        if (type == Type.OWN) {
            textPanel.setAlignmentX(Component.RIGHT_ALIGNMENT);
            nameLabel.setAlignmentX(Component.RIGHT_ALIGNMENT);
            textArea.setAlignmentX(Component.RIGHT_ALIGNMENT);
        } else {
            textPanel.setAlignmentX(Component.LEFT_ALIGNMENT);
            nameLabel.setAlignmentX(Component.LEFT_ALIGNMENT);
            textArea.setAlignmentX(Component.LEFT_ALIGNMENT);
        }

        // 先添加用户名标签，再添加文本区域
        textPanel.add(nameLabel);
        textPanel.add(textArea);

        // 添加子Panel到messagePanel
        if (type == Type.OWN) {
            this.add(avatarPanel, BorderLayout.EAST);
        } else {
            this.add(avatarPanel, BorderLayout.WEST);
        }
        this.add(textPanel, BorderLayout.CENTER);

        // 在创建面板后添加以下代码
        this.setBorder(BorderFactory.createEmptyBorder(5, 5, 5, 5));
        avatarPanel.setBorder(BorderFactory.createEmptyBorder(0, 5, 0, 5));

        // 动态计算 messagePanel 高度
        int messagePanelHeight = Math.max(avatarPanel.getPreferredSize().height, textPanel.getPreferredSize().height) + 40;

        // 设置最大尺寸为首选尺寸
        this.setMaximumSize(new Dimension(Integer.MAX_VALUE, messagePanelHeight));
        this.setAlignmentX(Component.LEFT_ALIGNMENT);

        // 添加 ComponentListener 以便在父 panel 大小变化时调整 MessagePanel 的大小
        this.addComponentListener(new ComponentAdapter() {
            @Override
            public void componentResized(ComponentEvent e) {
                adjustTextAreaSize();
                adjustMessagePanelSize();
            }
        });
    }

    public void updatePanel() throws IOException {
        this.removeAll(); // 清除所有组件

        this.initAvotorIcon(avatar);
        this.initNameLabel(userName);
        this.initTextArea(message);

        // 重新创建子Panel用于放置circleLabel
        avatarPanel = new JPanel();
        avatarPanel.setLayout(new BorderLayout());
        avatarPanel.add(avatarIcon, BorderLayout.NORTH);

        // 重新创建子Panel用于放置textArea
        textPanel = new JPanel();
        textPanel.setLayout(new BoxLayout(textPanel, BoxLayout.Y_AXIS));
        if (type == Type.OWN) {
            textPanel.setAlignmentX(Component.RIGHT_ALIGNMENT);
            nameLabel.setAlignmentX(Component.RIGHT_ALIGNMENT);
            textArea.setAlignmentX(Component.RIGHT_ALIGNMENT);
        } else {
            textPanel.setAlignmentX(Component.LEFT_ALIGNMENT);
            nameLabel.setAlignmentX(Component.LEFT_ALIGNMENT);
            textArea.setAlignmentX(Component.LEFT_ALIGNMENT);
        }

        // 先添加用户名标签，再添加文本区域
        textPanel.add(nameLabel);
        textPanel.add(textArea);

        // 添加子Panel到messagePanel
        if (type == Type.OWN) {
            this.add(avatarPanel, BorderLayout.EAST);
        } else {
            this.add(avatarPanel, BorderLayout.WEST);
        }
        this.add(textPanel, BorderLayout.CENTER);

        // 在创建面板后添加以下代码
        this.setBorder(BorderFactory.createEmptyBorder(5, 5, 5, 5));
        avatarPanel.setBorder(BorderFactory.createEmptyBorder(0, 5, 0, 5));

        // 动态计算 messagePanel 高度
        int messagePanelHeight = Math.max(avatarPanel.getPreferredSize().height, textPanel.getPreferredSize().height) + 40;

        // 设置最大尺寸为首选尺寸
        this.setMaximumSize(new Dimension(Integer.MAX_VALUE, messagePanelHeight));
        this.setAlignmentX(Component.LEFT_ALIGNMENT);

        // 重新调整大小
        adjustTextAreaSize();
        adjustMessagePanelSize();

        this.revalidate(); // 重新验证组件
        this.repaint(); // 重新绘制组件
    }

    private void adjustTextAreaSize() {
        // 动态计算文本宽度
        FontMetrics metrics = textArea.getFontMetrics(textArea.getFont());
        int textWidth = metrics.stringWidth(textArea.getText()) + 20; // 加上边距
        int maxWidth = this.parentPanel.getWidth() - 60; // 减去圆形标签和边距的宽度

        // 动态计算文本高度
        double lineHeight = metrics.getHeight() * 1.3;
        int lines = (int) Math.ceil((double) textWidth / maxWidth);
        int textHeight = (int) (lineHeight * lines) + 5; // 加上边距

        if (textWidth > maxWidth) {
            textWidth = maxWidth;
        }
        textArea.setPreferredSize(new Dimension(textWidth, textHeight));
        textArea.setMaximumSize(textArea.getPreferredSize());
    }

    private void adjustMessagePanelSize() {
        int messagePanelHeight = Math.max(avatarPanel.getPreferredSize().height, textPanel.getPreferredSize().height) + 20;
        this.setMaximumSize(new Dimension(Integer.MAX_VALUE, messagePanelHeight));
    }

    private void initTextArea(String message) {
        textArea = new JTextArea(message);
        textArea.setEditable(false);
        textArea.setFont(new Font("Microsoft YaHei", Font.PLAIN, 15)); // 设置支持中文的字体
        textArea.setBorder(new FlatRoundBorder()); // 设置圆角边框
        textArea.setLineWrap(true); // 自动换行

        adjustTextAreaSize();
    }

    private void initAvotorIcon(String base64Image) throws IOException {
        this.avatarIcon = new JLabel();
        avatarIcon.setPreferredSize(new Dimension(40, 40));
        avatarIcon.setOpaque(false);
        if (base64Image == null) // 默认的头像
            base64Image = "";
        BufferedImage image = ImageManager.base64ToCircularImage(base64Image);
        if (image != null) {
            Image scaledImage = ImageManager.resizeImage(image, 40, 40);
            ImageIcon icon = new ImageIcon(scaledImage);
            avatarIcon.setIcon(icon);
        } else {
            avatarIcon.setIcon(new Icon() {
                @Override
                public void paintIcon(Component c, Graphics g, int x, int y) {
                    g.setColor(Color.BLUE); // 圆形颜色
                    g.fillOval(x, y, getIconWidth(), getIconHeight());
                }

                @Override
                public int getIconWidth() {
                    return 40;
                }

                @Override
                public int getIconHeight() {
                    return 40;
                }
            });
        }
    }

    private void initNameLabel( String userName ) {
        // 添加用户名标签
        nameLabel = new JLabel(userName); 
        nameLabel.setFont(new Font("Microsoft YaHei", Font.PLAIN, 12));
    }


    public void setType(Type type) throws IOException {
        this.type = type;
        updatePanel();
    }

    public Type getType() {
        return type;
    }
}
